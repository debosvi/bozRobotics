...
int main(int ac, char** av) {
    ...
    QRosNodeSingleton::instance()->init(ac, av, node_name);
    
    QRosNodeSingleton::instance()->subscribe<std_msgs::UInt16>(topic, size, &test, SLOT(onReceive(const std_msgs::UInt16ConstPtr&)));
    QObject::connect(QRosNodeSingleton::instance(), SIGNAL(rosShutdown()), &app, SLOT(quit()));
